...
class Bond(Instrument):
    def __init__(self, face: float, coupon_payment: float, period_count: float, maturity: float):
        self.face = face
    ...
    @property
    def cash_flows(self) -> Iterator[CashFlow]:
        return chain((CashFlow(t * self.period, self.coupon_payment)))

    def present_worth(self, yield_: float) -> float:
        return self.coupon_payment * pa(yield_ / self.period_count, self.maturity * self.period_count) + self.face \
