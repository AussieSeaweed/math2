...
class PS2TestCase(ExtendedTestCase):
    ...
    def test_3(self):
        ...
        for m, p, a, b in cases:
            self.assertAlmostEqual(np.float_(fsolve(lambda y: p * EfInt(y).to_factor(m / 12) - 100, np.array(0))), a)
            self.assertAlmostEqual(np.float_(fsolve(lambda y: p * ContInt(y).to_factor(m / 12) - 100, np.array(0))), b)

        self.assertAlmostEqual(np.interp(7, (4, 8), (0.03029791, 0.03647384)), 0.0349298575)
        self.assertAlmostEqual(np.interp(7, (4, 8), (0.02984799, 0.03582441)), 0.034330305)
    ...
    def test_4(self):
        ...
        self.assertAlmostEqual(100 * ContInt(r[4]).to_factor(-4. / 12), 98.68531348348684)
...
class PS3TestCase(TestCase):
    ...
    def test_3(self):
        ...
        self.assertAlmostEqual(pw(Bond.from_rate(
            100, NomInt(0.07, 2), 2, 2.25).cash_flows, NomInt(0.05, 2)), 102.65033622528411)
        self.assertAlmostEqual(np.float_(fsolve(
    ...
    def test_4(self):
        ...
        a0 = a * pa(i, g, n) * (gp - r) / (np.exp(n * (gp - r)) - 1)
...
class PS5TestCase(ExtendedTestCase):
    ...
    def test_7(self):
        ...
        self.assertAlmostEqual(np.float_(fsolve(lambda x: x * s * pa(i, n, g) - t * pf(i, n), np.array(0))),
                               0.09187408282061407)
...
class PS7TestCase(ExtendedTestCase):
    def test_1(self):
        ...
        self.assertIterableAlmostEqual(DblDeclBalDeprec(basis, salvage, life).books, (92000, 46000, 23000, 11500, 5750))
    ...
    def test_3(self):
        ...
        self.assertAlmostEqual(np.float_(fsolve(
            lambda x: StrLineDeprec(basis, x, life).book(6) - book, np.array(0))), 27666.66666666667)
        self.assertAlmostEqual(np.float_(fsolve(
